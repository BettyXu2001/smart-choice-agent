...
2026-09-06：用户明确回复“实施”后进入实现。已完成后端 timeline、状态差异、过滤/排名诊断、模型/fallback 记录和前端时间轴；已完成 Python compileall 与 JS syntax check。当前环境缺少 pytest、SQLAlchemy、FastAPI，未能执行 pytest 和真实后端/浏览器验收。
...
- [x] 实现 collector、差异、引用、脱敏，验证旧事件兼容。
- [x] 接入请求命令、Agent 起止、提交，验证修改前基线与幂等。
- [x] 接入检索、证据、过滤、排名、饮食选择/三餐/假设，验证原结果不变。
- [x] 接入模型输入输出、采用、fallback，验证失败/规则模式区分。
- [x] 实现推荐变化与关联，验证归因边界。
- [x] 实现时间轴、展开、旧记录、错误、窄屏交互。
- [ ] 完成专项/全量自动验证、浏览器验收，记录未验证项。
- [x] 检查 diff，更新 CHANGELOG 和验收记录。
