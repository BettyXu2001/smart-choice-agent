...
- [x] 扩展 `DecisionState` 和新增历史、最终选择、用户资料 Schema。
- [x] 新增 `UserProfileRecord` 和用户资料仓储。
- [x] 扩展 `DecisionRepository`，支持按用户列出历史决策并生成摘要。
- [x] 新增历史决策 API：列表、详情、保存最终选择、清除最终选择。
- [x] 新增用户资料 API：读取和保存。
- [x] 调整顶部主导航为 `首页 | 历史决策 | 我的资料`。
- [x] 增加开发者工具菜单、Developer Mode 开关和 Debug 页面。
- [x] 实现历史决策列表、历史详情和最终选择记录 UI。
- [x] 实现我的资料页面，并保留个人餐食库入口。
- [x] 调整首页普通模式，移除显性的工程入口。
- [x] 增加历史决策和用户资料测试。
- [x] 更新 `CHANGELOG.md`。
- [x] 执行自动验证和关键行为验证。
