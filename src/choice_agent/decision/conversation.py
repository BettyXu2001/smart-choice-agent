...
FIELD_DEFINITIONS = {
    ...
    "generic": {"background": ("已有基础", "text", "", "路径适配参考"), "weeklyHours": ("每周可用时间", "number", "小时", "时间投入参考；不承诺完成时长"), "target": ("决策目标", "text", "", "比较目标"), "priority": ("更看重", "text", "", "定性比较参考；有评分时调整对应权重")},
}
PRIORITIES = {"performance": ["性能", "游戏", "剪辑"], "portability": ["便携", "轻薄"], "battery": ["续航"], "durability": ["耐用"], "support": ["售后"], "price": ["价格", "性价比", "便宜"], "relaxation": ["轻松", "放松"], "crowd_level": ["人少", "安静"], "nature": ["自然", "风景"], "budget": ["省钱", "预算"], "travel_hours": ["交通", "路程"], "fit": ["匹配", "成长"], "cost": ["成本"], "risk": ["风险", "稳定"]}
...
def sync_dependencies(decision, defaults=None):
    ...
    for field, attribute in [("budget", "price" if decision.domain == "shopping" else "budget"), ("maxTransitHours", "travel_hours")]:
        item = current.get(field, {})
        if item.get("value") is not None and item.get("confirmed"):
